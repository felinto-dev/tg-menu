...
describe(MenuPathParser.name, () => {
  describe('sanitization', () => {
    it('should sanitize data', () => {
      ...
      expect(stu.template).toEqual('/producer/accounts/');
    });

    it('add support to main "/" menu', () => {
      const stu = new MenuPathParser(RequestMethod.GET, '/');
      expect(stu.template).toEqual('/');
    });

    it('should recognize as main menu if user does not put nothing as template', () => {
      const stu = new MenuPathParser(RequestMethod.GET, '');
      expect(stu.template).toEqual('/');
    });
  });
